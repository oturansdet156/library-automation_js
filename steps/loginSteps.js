...
When("user enters invalid password libraryPassword", async function () {
  await PageManager.loginPage.enterPassword(string);
});

Then("user sees a {string} error pop-up", async function (string) {
  await expect(PageManager.loginPage.loginErrorMessage).toHaveText(string);
  await BrowserUtility.verifyMessage(
    PageManager.loginPage.loginErrorMessage.innerText(),
    string
  );
